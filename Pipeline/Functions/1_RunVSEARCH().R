RunVSEARCH<-function(SimilarityThreshold=0.97, clustering="curatedESV", TableToMergeTo=LuluOutput1, multithread=multithread) { 
    ...
    if (clustering=="ESV") {
        ...
        (print("unrecognised clustering request")) 
    }

    write.fasta(sequences=Sequences, names=Sequences, file.out=file.path(path, "IntermediateOutputs", paste0(dataname, "_", clustering, "_sequences_NamedAsSeqs.fasta")))

    # create matchlist from fasta
    sequencesfile<-paste0(dataname, "_", clustering, "_sequences_NamedAsSeqs.fasta")
    system(command= paste0(
        "mkdir -p ", 
        file.path(path,"IntermediateOutputs", paste0(dataname , "_vsearchOTUs")),
        ...
        " --clusterout_id ",
        " --clusters ", 
        file.path(path,"IntermediateOutputs", paste0(dataname , "_vsearchOTUs"), "Cluster"),
        " --uc ", 
        file.path(path,"IntermediateOutputs", paste0(dataname , "_vsearchdata")),
        " --threads ", 
        mc.cores,
        " --id ", 
        SimilarityThreshold,
    ...
    OTULabeledSeqs<-data.frame(OTU=paste0("OTU_",vsearchclusters$V2), Sequence=vsearchclusters$V9, OTUrepresentativeSequence= (vsearchclusters$V1=="S") )

    SeqDataTable<-MergeClusteringResultsintoDataTable(TableToMergeTo=TableToMergeTo, OTULabeledSeqs=OTULabeledSeqs)
