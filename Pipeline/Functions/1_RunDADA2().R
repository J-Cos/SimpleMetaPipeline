RunDADA2<-function(
        ...
        for (Run in (NumberCompletedRuns+1):NumberOfRuns) {
            ...
            if(ReadType=="Paired-end"){
                ...
                out <- filterAndTrim(
                    ...
                    fnRs, 
                    filtRs, 
                    truncLen=truncLen[[Run]],
                    trimLeft=trimLeft[[Run]], 
                    maxN=maxN, 
                    maxEE=maxEE, 
                    truncQ=truncQ, 
                    rm.phix=TRUE, 
                ...
                RunDadaPlots[[Run]][[1]]<-plotQualityProfile(fnFs) 
                RunDadaPlots[[Run]][[2]]<-plotQualityProfile(fnRs) 
                RunDadaPlots[[Run]][[3]]<-plotQualityProfile(filtFs) 
                RunDadaPlots[[Run]][[4]]<-plotQualityProfile(filtRs) 
    ...
    ESVtable<- removeBimeraDenovo(ESVtable, method="consensus", multithread=multithread, verbose=TRUE)
